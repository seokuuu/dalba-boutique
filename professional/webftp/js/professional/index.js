/*
 * index.js - d'Alba PROFESSIONAL 메인 페이지 스크롤 애니메이션
 * signature index.js 복제본. window.professional 네임스페이스 사용.
 * 사용 클래스: .hero__container / .fade-in-text / .horizontal-scroll__item / .discover (main.html과 일치)
 */
(function (window) {
    const professional = window.professional || null;
    if (!professional) {
        console.error("[professional/index.js] professional namespace not found!");
        return;
    }

    const { utils, modules } = professional;
